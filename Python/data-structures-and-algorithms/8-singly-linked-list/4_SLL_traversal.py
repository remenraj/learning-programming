...
class SinglyLinkedList:
    ...
    def traverse_SLL(self) -> None:
        """Traverse Singly Linked List"""

        # checking for head, if head has reference, then there is a node present, else there is no node connected
        if self.head is None:
            print("Singly Linked List doesnot exist")

        # traverse through the list
        else:
            # starting from the first node
            node = self.head  # head stores the physical location of first node

            while node is not None:
                print(node.value)  # printing the value
...
# traversing through the singly linked list
