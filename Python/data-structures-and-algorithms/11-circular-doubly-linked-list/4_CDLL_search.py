...
class CircularDoublyLinkedList:
    ...
    def search_CDLL(self, node_value):
        """Search for a node in the circular doubly linked list"""
        # checking if there is no nodes in the linked list
        if self.head is None:
            return "There is no nodes in the linked list"

        else:
            # iterating over the linked list to find the node to be searched
            # temp_node will be the node to be searched
            temp_node = self.head
            
            while temp_node:
                # checking if the value of the current node is equal to the node_value
                if temp_node.value == node_value:
                    return temp_node.value

                # breaking the loop if the node is at the tail
...
if __name__ == "__main__":
    ...
    print(circular_DLL.search_CDLL(node_value=5))
    print(circular_DLL.search_CDLL(node_value=55))